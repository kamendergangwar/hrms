...
//   // 1. Use a single GestureHandlerRootView for the entire component
//   return (
//     <GestureHandlerRootView style={{ flex: 1 }}>
//       {/* 2. Use conditional rendering inside the root view */}
//       {isLoading ? (
//         <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }} className="dark:bg-black">
...
//                 className="flex-1 dark:text-white h-14" // Added height for consistency
...
//             >
//               <View className='p-4 ml-2 items-center bg-white dark:bg-black-100 rounded-lg h-14 justify-center'>
...
const ServiceItem = ({ title, icon, href, isGrid, disabled }) => {
  ...
  return (
    <TouchableOpacity
      className={`p-2 ${isGrid ? "w-[50%]" : "w-full"} ${disabled ? "opacity-40" : ""}`}
      onPress={() => router.push(href)}
      disabled={disabled}
    >
      {isGrid ? (
        ...
        <View className="bg-white dark:bg-black-100 p-6 rounded-xl flex-row items-center">
          <Image source={icon} className="w-12 h-12 shadow-lg" />
          <Text className="text-md font-semibold ml-5 dark:text-gray-100">{title}</Text>
        </View>
      )}
        ...
        <View className="flex-1 dark:bg-black">
          <View className="px-2 m-4 mb-1 flex-row">
            <View style={{ flex: 4 }} className="flex-row items-center bg-white dark:bg-black-100 rounded-xl px-4">
              <TextInput
                placeholder="Search Services"
                className="flex-1 dark:text-white h-14"
                placeholderTextColor={colorScheme === 'light' ? "black" : "#CDCDE0"}
                value={searchQuery}
                onChangeText={setSearchQuery}
              />
            ...
            >
              <View className="p-4 ml-2 items-center bg-white dark:bg-black-100 rounded-lg h-14 justify-center">
                <Icon name={isListView ? "grid" : "list"} size={20} color={colorScheme === 'light' ? "black" : "#CDCDE0"} />
              </View>
            </TouchableOpacity>
          </View>
          <ScrollView contentContainerStyle={{ paddingHorizontal: 16 }}>
            <View className={`flex-row flex-wrap ${isListView ? "flex-col" : ""}`}>
              {filteredServices.map(service => (
                <ServiceItem
                  key={service.title}
                  title={service.title}
