...
const AuthProvider = ({ children }) => {
  ...
  console.log("AuthProvider Loaded API URL:", process.env.EXPO_PUBLIC_API_BASE_URL);
...
const authenticate = async (email, password, deviceToken) => {
    ...
    try {
      ...
      const requestBody = {
        email: email,
        password: password,
        deviceToken: deviceToken?.data || null,
        platform: platform || null,
      };

      const response = await fetch(`${apiUrl}/login`, {
        // 2. IMPORTANT: Specify the POST method.
        method: 'POST',

        // 3. IMPORTANT: Set the headers to tell the server you're sending JSON.
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
        },

        // 4. IMPORTANT: Convert the JavaScript object into a JSON string.
